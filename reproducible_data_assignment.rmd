...
#Install and Load the required packages for the assignment. 
...
#Load and view the raw penguin data 
```{r}
...
### Create a Hypothesis - I have provided a null and alternative hypothesis based on variables from the dataset
...
My aim was to investigate the correlation between Culmen depth and Culmen length between male and female penguins. There appears to be sexual dimorphism amongst Antarctic penguins, with male culmen lengths, on average, being larger than female culmens (Gorman et al. 2014). I created an alternative hypothesis for this dataset stating that " There is a significant difference between culmen length and culmen depth between male and female Antarctica penguins."

Firstly, I used an exploratory scatterplot to make initial judgments on the distribution of data and to identify potential relationships between my chosen variables. The colour key for the sexes enables the visualisation of lower female culmen depths and lengths in comparison the the males, on average. 

To investigate my hypothesis, I used a linear regression analysis to assess the relationship between two continuous variables, 'Culmen depth' and 'Culmen length' as well as the categorical variable, 'Sex'. 'Sex' as an independent variable enables the analysis of the differences of culmen length and depth in males and gemales. Overall, this is an appropriate model as the significance of sex on culmen length and depth can be assessed, allowing us to evaluate the hypotheses. 

Based on my linear regression analysis of my data, there appears to be a significant difference between culmen length and culmen depth between males and females. The positive coefficient estimate of 5.4445 implies that males have a longer culmen length than their female counterparts.  Furthermore,the overall  p-value of < 2.2e-16 is below 0.05, indicating statistical significance and the null hypothesis is rejected.
